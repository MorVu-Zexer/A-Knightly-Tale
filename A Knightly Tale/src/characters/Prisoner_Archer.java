/**
 * Program Name: Prisoner_Archer.java
...
public class Prisoner_Archer extends Character {

	// Variables
	private BufferedImage spriteSheet = null;
	
	public Prisoner_Archer() {
		// Base stats
		super(0, 0, 0, "Prison Archer", 10, 7, 2, 6, 5, 5, false, false);
		bufferImages();
		super.setSprite(spriteSheet.getSubimage(super.getW()*0, super.getH()*0, super.getW(), super.getH())); // Idle 1
	}
	
	public Prisoner_Archer(int x, int y, int size, int hp, int str, int mag, int def, int res, int mov, boolean ally) {
		super(x, y, size, "Prison Archer", hp, str, mag, def, res, mov, false, false);
		bufferImages();
		super.setSprite(spriteSheet.getSubimage(super.getW()*0, super.getH()*0, super.getW(), super.getH())); // Idle 1
	}
	...
	private void bufferImages() {
		try {
			spriteSheet = ImageIO.read(new File("src/images/characters/prisoner_archer.png"));
		} catch (IOException e) {
